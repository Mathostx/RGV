    ...
    <div class="doc-card severity-${doc.severity}">
      <div class="doc-header">
        <div class="doc-severity-badge severity-badge-${doc.severity}">
          ${doc.severity === 'critical' ? (currentLang === 'es' ? 'URGENTE' : 'URGENT') : doc.severity === 'warning' ? (currentLang === 'es' ? 'IMPORTANTE' : 'IMPORTANT') : 'INFO'}
        </div>
        <div class="doc-form">${doc.form}</div>
      </div>
      <h3 class="doc-name">${doc.name[currentLang]}</h3>
      <div class="doc-section">
        <div class="doc-label">${currentLang === 'es' ? '¿Qué es?' : 'What is it?'}</div>
        <p>${doc.what[currentLang]}</p>
      </div>
      <div class="doc-section doc-action">
        <div class="doc-label">${currentLang === 'es' ? '¿Qué hacer?' : 'What to do?'}</div>
        <p>${doc.do[currentLang]}</p>
      </div>
    </div>
  `).join('');
}
  ...
  grid.innerHTML = guides.map((g, i) => `
    <div class="guide-card">
      ...
      <ol class="guide-steps">
        ${g.steps[currentLang].map(step => `<li>${step}</li>`).join('')}
      </ol>
    </div>
  `).join('');
}
...
function renderLawyers(city = 'All') {
  ...
  grid.innerHTML = filtered.map(l => `
    <div class="lawyer-card ${l.free_service ? 'lawyer-free-service' : ''}">
      ${l.free_service ? `<div class="free-service-badge">${currentLang === 'es' ? 'SERVICIO GRATUITO' : 'FREE SERVICE'}</div>` : ''}
      <div class="lawyer-header">
        <div class="lawyer-avatar">${l.name.charAt(0)}</div>
        <div>
          <div class="lawyer-name">${l.name}</div>
          <div class="lawyer-city">${l.city}, TX · ${l.years} ${currentLang === 'es' ? 'años exp.' : 'yrs exp.'}</div>
        </div>
      </div>
      <div class="lawyer-specialty">${l.specialty}</div>
      <div class="lawyer-address">📍 ${l.address}</div>
      ${l.free_consult ? `<div class="free-consult-badge">${currentLang === 'es' ? '✓ Consulta Gratuita' : '✓ Free Consultation'}</div>` : ''}
...
function renderFilters() {
  ...
  el.innerHTML = cities.map(city => `
    <button class="city-filter-btn ${city === 'All' ? 'active' : ''}" data-city="${city}" onclick="renderLawyers('${city}')">
